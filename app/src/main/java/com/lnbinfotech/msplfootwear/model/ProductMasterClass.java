package com.lnbinfotech.msplfootwear.model;

//Created by lnb on 10/4/2017.

public class ProductMasterClass {

    public String Cat1,Cat2,Cat3,Cat4,Cat5,Cat6,Final_prod,Uom,Srate,Prate,Status,MRPRate,ProductId,Cat7,Cat8,MinStkQty,MaxStkQty,GSTGroup,HSNCode,Cat9;
    public int Product_id,Branchid,NoOfPices,Company_Id;

    public String getCat1() {
        return Cat1;
    }

    public void setCat1(String cat1) {
        Cat1 = cat1;
    }

    public String getCat2() {
        return Cat2;
    }

    public void setCat2(String cat2) {
        Cat2 = cat2;
    }

    public String getCat3() {
        return Cat3;
    }

    public void setCat3(String cat3) {
        Cat3 = cat3;
    }

    public String getCat4() {
        return Cat4;
    }

    public void setCat4(String cat4) {
        Cat4 = cat4;
    }

    public String getCat5() {
        return Cat5;
    }

    public void setCat5(String cat5) {
        Cat5 = cat5;
    }

    public String getCat6() {
        return Cat6;
    }

    public void setCat6(String cat6) {
        Cat6 = cat6;
    }

    public String getFinal_prod() {
        return Final_prod;
    }

    public void setFinal_prod(String final_prod) {
        Final_prod = final_prod;
    }

    public String getUom() {
        return Uom;
    }

    public void setUom(String uom) {
        Uom = uom;
    }

    public String getSrate() {
        return Srate;
    }

    public void setSrate(String srate) {
        Srate = srate;
    }

    public String getPrate() {
        return Prate;
    }

    public void setPrate(String prate) {
        Prate = prate;
    }

    public String getStatus() {
        return Status;
    }

    public void setStatus(String status) {
        Status = status;
    }

    public String getMRPRate() {
        return MRPRate;
    }

    public void setMRPRate(String MRPRate) {
        this.MRPRate = MRPRate;
    }

    public String getProductId() {
        return ProductId;
    }

    public void setProductId(String productId) {
        ProductId = productId;
    }

    public String getCat7() {
        return Cat7;
    }

    public void setCat7(String cat7) {
        Cat7 = cat7;
    }

    public String getCat8() {
        return Cat8;
    }

    public void setCat8(String cat8) {
        Cat8 = cat8;
    }

    public String getMinStkQty() {
        return MinStkQty;
    }

    public void setMinStkQty(String minStkQty) {
        MinStkQty = minStkQty;
    }

    public String getMaxStkQty() {
        return MaxStkQty;
    }

    public void setMaxStkQty(String maxStkQty) {
        MaxStkQty = maxStkQty;
    }

    public String getGSTGroup() {
        return GSTGroup;
    }

    public void setGSTGroup(String GSTGroup) {
        this.GSTGroup = GSTGroup;
    }

    public String getHSNCode() {
        return HSNCode;
    }

    public void setHSNCode(String HSNCode) {
        this.HSNCode = HSNCode;
    }

    public String getCat9() {
        return Cat9;
    }

    public void setCat9(String cat9) {
        Cat9 = cat9;
    }

    public int getProduct_id() {
        return Product_id;
    }

    public void setProduct_id(int product_id) {
        Product_id = product_id;
    }

    public int getBranchid() {
        return Branchid;
    }

    public void setBranchid(int branchid) {
        Branchid = branchid;
    }

    public int getNoOfPices() {
        return NoOfPices;
    }

    public void setNoOfPices(int noOfPices) {
        NoOfPices = noOfPices;
    }

    public int getCompany_Id() {
        return Company_Id;
    }

    public void setCompany_Id(int company_Id) {
        Company_Id = company_Id;
    }
}
